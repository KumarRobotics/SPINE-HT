...
def generate_launch_description():
    ...
    launch_nav2 = PathJoinSubstitution([pkg_spine_ht, "launch", "navigation_launch.py"])

    nav2_config_name = ["nav2_husky.yaml"]

    nav2_config = PathJoinSubstitution([pkg_spine_ht, "config", nav2_config_name])
    clearpath_static_transforms = PathJoinSubstitution(
        [pkg_spine_ht, "launch", "clearpath_static_transforms.launch.py"]
    )
    ...
    namespaced_group = GroupAction(
        actions=[
            ...
            Node(
                package="spine_ht_ros",  # Replace with your package name
    ...
    record_launch_path = PathJoinSubstitution(
        [pkg_spine_ht, "launch", "record_husky.launch.py"]
    )
