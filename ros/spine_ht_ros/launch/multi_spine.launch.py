...
def generate_launch_description():
    ...
    declare_config = DeclareLaunchArgument(
        "robot_config",
        default_value="/home/zac/projects/dcist/dcist_ws/src/ros/spine_ht_ros/config/spine_configs/spine_ht.yaml",
        description="config",
    )
    ...
    # robot_config = PathJoinSubstitution([pkg_spine_ht, "config", "spine_ht.yaml"])
...
def get_log_dir() -> str:
    ...
    bag_name = str(
        log_dir
        / f"{n_bags:03d}_spine-ht-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    )
    return bag_name
