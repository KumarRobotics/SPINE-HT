...
from spine_ht_ros.vlm_manager import VLMReq
...
class VLMServiceTranslator(Node):
    ...
    def _req_cbk(self, req: VLMRequest) -> None:
        ...
        if req.idx not in self._vlm_query_dict:
            self.get_logger().info(
                f"[vlm service translator] Processing new request {req.idx}"
            )
            self._process_request_async(req)
        else:
            self.get_logger().info(
                f"[vlm service translator] Skipping duplicate request {req.idx}"
            )
    ...
    def _query_vlm_async(self, req: VLMRequest):
        ...
        self.get_logger().info(
            f"[vlm service translator] calling VLM service async for idx {req.idx} with query: {request.query}"
        )
    ...
    def _handle_vlm_response(self, future, req: VLMRequest):
        ...
        self.get_logger().info(
            f"[vlm service translator] handling vlm resp for {req.idx}"
        )
        ...
        try:
            ...
            self.get_logger().info(
                f"[vlm service translator] VLM returned success={success} for {req.idx} with answer: {answer}"
            )
        except Exception as e:
            self.get_logger().error(
                f"[vlm service translator] VLM service call failed for {req.idx}: {e}"
            )
            success = False
            answer = ""
    ...
    def _start_response_publishing(self, resp_msg: VLMResponse, attempt: int):
        ...
        self.get_logger().info(
            f"[vlm service translator] Starting repub for idx {resp_msg.idx} attempt {attempt}"
        )

        if attempt >= self._max_pub_count:
            self.get_logger().info(
                f"[vlm service translator] Max attempts reached for {resp_msg.idx}"
            )
        ...
        # Check if we got ack
        if (
            resp_msg.idx in self._vlm_query_dict
            and self._vlm_query_dict[resp_msg.idx].ack
        ):
            self.get_logger().info(
                f"[vlm service translator] Got ack for {resp_msg.idx}, stopping retries"
            )
            self._cleanup_req(attempt)
            return

        self.get_logger().info(
            f"[vlm service translator] Publishing attempt {attempt+1}/{self._max_pub_count} for {resp_msg.idx}"
        )
        self._status_update_pub.publish(resp_msg)

        # Schedule next attempt in 5 seconds (non-blocking)
        timer = self.create_timer(
            5.0, lambda: self._timer_callback(resp_msg, attempt + 1)
        )
