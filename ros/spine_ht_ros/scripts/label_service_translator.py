...
from spine_ht_ros.set_label_manager import LabelReq
...
class LabelServiceTranslator(Node):
    ...
    def _req_cbk(self, req: LabelRequest) -> None:
        self.get_logger().info(f"[label service translator] got req for {req.idx}")
    ...
    def _call_set_labels_async(self, req: LabelRequest):
        ...
        self.get_logger().info(
            f"[label service translator] calling set_labels async for {req.idx}"
        )
    ...
    def _handle_service_response(self, future, req: LabelRequest):
        try:
            ...
            self.get_logger().info(
                f"[label service translator] service returned {success} for {req.idx}"
            )
    ...
    def _start_response_publishing(self, resp_msg: LabelResponse, attempt: int):
        ...
        if (
            resp_msg.idx in self._label_query_dict
            and self._label_query_dict[resp_msg.idx].ack
        ):
            self.get_logger().info(
                f"[label service manager] Got ack for {resp_msg.idx}, stopping"
            )
            return

        self.get_logger().info(
            f"[label service manager] Publishing response {attempt+1}/{self._max_pub_count} for {resp_msg.idx}"
        )
        self._status_update_pub.publish(resp_msg)

        # Schedule next attempt (non-blocking)
        timer = self.create_timer(
            5.0, lambda: self._start_response_publishing(resp_msg, attempt + 1)
        )
        timer.cancel()
