...
class TwistConverter(Node):
    def __init__(self):
        ...
        self._publish_stamped = (
            self.get_parameter("publish_stamped").get_parameter_value().bool_value
        )

        # Subscribe to TwistStamped
        self.subscription = self.create_subscription(
            TwistStamped, "cmd_vel_nav", self.twist_stamped_callback, 10
        )
        ...
        self.publisher = self.create_publisher(
            TwistStamped, "cmd_vel", 10  # Will be remapped to /warthog1/cmd_vel
        )
