...
from teaming_msgs.srv import Query, SetLabels
...
class SpotAutonomyManager(Node):
    def __init__(self):
        ...
        self._pose_on_start = SE2Pose(
            pose_on_start[0], pose_on_start[1], pose_on_start[2]
        )
        ...
        # navigation
        self._nav_component = SpotNavigationComponent(
            parent_node=self, spot_client=self._spot_client
        )
        ...
        self.timer = self.create_timer(
            0.1, self._publish_odom, callback_group=timer_cbk_group
        )
...
class SpotNavigationComponent:
    ...
    def navigate_and_wait(
        ...
        self._log_info(
            f"goal world: {goal_world}, goal: {goal}, start: {pose_on_start}"
        )
        ...
        success, is_taken_over = self._spot_client.safe_navigate_to_pose(
            x=x, y=y, yaw=yaw, timeout_sec=timeout_sec, goal_tol=self._goal_tol
        )
