...
from spine_ht_ros.nav_manager import NavigationComponentAction, NavigationComponentTopic
from spine_ht_ros.tracker_client import TrackerClientComponent
from spine_ht_ros.vlm_manager import VLMManagerAction, VLMManagerTopic
from teaming_msgs.srv import Mission

from spine_ht.planner_logging import get_logger
from spine_ht.spine import SPINE, GraphHandler
...
class SPINE_node(Node):
    def __init__(self):
        ...
        if use_actions:
            self._nav_component = NavigationComponentAction(
                self, frame_id=target_frame, robot_name="robot"
            )
        ...
        self._logger = get_logger(
            name="spine_ht", output="both", filename=self._get_log_fname()
        )
    ...
    def _get_log_fname(self):
        # TODO refactor
        log_dir = Path.home() / "logs/spine-ht-logs"
    ...
    def _mission_cbk(
        ...
        while True:
            ...
            self.get_logger().info("[spine node] finished planning iteration")
