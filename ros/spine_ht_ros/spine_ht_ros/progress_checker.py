...
class ProgressChecker:
    ...
    def has_timedout(self) -> bool:
        ...
        if not could_lookup:
            self._log_info("could not look up transform")
            return True
        ...
        if min_dist_to_history > self._dist_tol:
            ...
            self._log_info(
                f"[WARNING] Robot has not moved more than {self._dist_tol} in {duration_sec}"
            )
    ...
    def get_min_dist_to_buffer(self, pos: np.ndarray) -> float:
        ...
        return np.linalg.norm(
            pos - np.array(self._history_buffer).reshape(-1, 2), axis=-1
        ).min()
    ...
    def lookup_robot_pos(self) -> Tuple[np.ndarray, bool]:
        try:
            transform = self._tf_buffer.lookup_transform(
                self._world_frame,
                self._robot_frame,
                rclpy.time.Time(),  # Latest available time rospy.Time(0)
            )
