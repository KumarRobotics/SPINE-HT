...
class TrackerClientComponent:
    ...
    def _track_cbk(self, track: Track) -> None:
        ...
        track_point = np.array([track.pose.pose.position.x, track.pose.pose.position.y])
        ...
        self._log_info(
            f"assigned incoming track {track.label} to {parent} at {region_node_locs[closest_region_idx]}"
        )
