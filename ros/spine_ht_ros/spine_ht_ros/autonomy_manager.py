...
from spine_ht_ros.action_manager import ActionManager
from spine_ht_ros.msg_handler import MessageHandler
from spine_ht_ros.tracker_client import TrackerClientComponent

from spine_ht.multi_robot_graph import MultiRobotGraphHandler
from spine_ht.spatial_tools import SE2Transforms
from spine_ht.spine import GraphHandler
from spine_ht.spine.mapping.frontiers import FrontierExtractor
from spine_ht.spine.util import UpdatePromptFormer
...
class AutonomyManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._log_info("constructed behavior library")
        ...
        self._log_info("initialized costmap cbk")

        # TODO need to clean this up
        self._uav_goal_pub = self._parent_node.create_publisher(
            PointStamped, "/to_titan/goal", qos_profile=qos_profile
        )
    ...
    def _costmap_cbk(self, costmap_msg: OccupancyGrid) -> None:
        ...
        self._frontier_extractor.update_costmap(
            costmap=costmap,
        ...
        for behavior, args in behavior_requests:
            ...
            if behavior.startswith("uav"):
                if behavior == "uav_map_region":
                    ...
                    goal_msg = self._build_pointstamped(
                        coords_world[0], coords_world[1]
                    )
                ...
                self._log_info(
                    f"[build behaviors] WARNINg skipping {behavior}, {args}\n\n"
                )
                continue
