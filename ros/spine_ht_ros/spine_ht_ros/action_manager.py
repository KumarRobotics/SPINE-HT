...
from spine_ht_ros.msg_handler import BehaviorRequestData, MessageHandler
...
from spine_ht.spine.mapping.frontiers import Node as GraphNode
...
class ActionManager:
    ...
    def _parse_attempt_navigate_results(self, results: List[dict], metadata) -> bool:
        ...
        self._parse_goto_region_results(results=nav_results, metadata=metadata)
    ...
    def _explore_to(self, goal_x: float, goal_y: float) -> bool:
        ...
        if not success:
            self._log_info("Could not add frontier. Breaking.")
            return False
    ...
    def _add_frontier(self, goal: np.ndarray) -> Tuple[bool, List[GraphNode]]:
        if self._frontier_extractor.filtered_costmap_with_info is None:
            self._log_info("ERROR no costmap")
            return False, None
    ...
    def _build_goto_region_request(
        ...
        if nav_start_location == goal_region and target_yaw is None:
        ...
        for node in path:
            ...
            if node == goal_region and target_yaw is not None:
