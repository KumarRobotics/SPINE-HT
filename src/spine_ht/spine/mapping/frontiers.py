...
class FrontierExtractor:
    def __init__(
        ...
        self.logger = get_logger(name="frontiers")
    ...
    def get_feasible_frontier(
        ...
        if search_success:
            ...
            self.logger.info("no best fit frontier found")
    ...
    def get_regions_in_costmap_idx(self, costmap_info: CostMapWithInfo):
        ...
        for i, region in enumerate(self.region_node_locs):
            in_costmap, log = self.is_pt_in_costmap(
                region.squeeze(), costmap_info=costmap_info
            )
    ...
    def find_neighbors_via_linesearch(
        ...
        for potential_neighbor_ind in potential_neighbors_inds:
            if self.line_is_free(
                ...
                # TODO cast to np str for some reason
                neighbor_ids.append(str(self.region_nodes[potential_neighbor_ind]))
    ...
    def get_frontiers(
        ...
        if self.filtered_costmap_with_info is None:
            return frontiers, ""

        self.logger.debug("about to get frontiers")
        self.compute_region_nodes()
        ...
        print(f"our neighbors: {neighbors}")
        frontiers.append(
            Node(
                id=f"discovered_region_{self.n_frontiers}",
                location=[float(best_fit_frontier[0]), float(best_fit_frontier[1])],
                neighbors=neighbors,
            )
        )
        self.n_frontiers += 1
