...
def parse_graph(
    ...
    flip_coords: Optional[bool] = False,
) -> Tuple[nx.Graph, str]:
    ...
    # because we may modify
    data = copy.deepcopy(data)

    G = nx.Graph()
    for node in data["objects"]:
        # print(f"node: {node}, coords: {node['coords']}")
...
class GraphHandler:
    def __init__(self, graph_path: str, init_node: Union[None, str] = None) -> None:
        if graph_path == "":
            self.graph = nx.Graph()
            self.initial_graph_as_json = "{}"
            self._current_location = ""
        else:
            with open(graph_path) as f:
                data = json.load(f)

            if "current_location" in data and init_node is None:
                init_node = data["current_location"]

            self.graph, self.initial_graph_as_json = parse_graph(data)
            self._current_location = init_node
            self.initial_graph_as_json = self.to_json_str()

    def get_induced_subgraph(self, nodes: List[str]) -> nx.Graph:
        neighbors = []
        for node in nodes:
            if node in self.graph:
                print(self.get_neighbors(node), node)
                neighbors.extend(self.get_neighbors(node))
        handler = GraphHandler("")
        nx_subgraph = self.graph.subgraph(neighbors + nodes)
        handler.from_nx(nx_subgraph)

        return handler

    def from_nx(self, g: nx.Graph, current_location: Optional[str] = "") -> None:
        self.graph = g
    ...
    def reset(
        self,
        graph_as_dict: Dict[str, Dict[str, Union[str, List]]],
        ...
        try:
            data = graph_as_dict  # json.loads(graph_as_json)

            # TODO, logic is obtuse
            # priority is current location -> incoming argument -> value in data

            if self._current_location is None or self._current_location == "":
                self._current_location = current_location

            self.graph, self.initial_graph_as_json = parse_graph(
                data,
                rotation=rotation,
    ...
    def path_exists_from_current_loc(self, target: str) -> bool:
        assert self._current_location is not None, "current location is unknown"
        return nx.has_path(self.graph, self._current_location, target)
    ...
    def get_node_coord(self, node: str) -> np.ndarray:
        ...
        return np.array(self.graph.nodes[node]["coords"])
    ...
    def get_closest_reachable_node(
        ...
        if current_node is None:
            assert self._current_location is not None, "current_location must be set"
...
if __name__ == "__main__":
    g = GraphHandler(
        "/home/zac/projects/dcist/src/spine-ht/ros/spine_ht_ros/data/perch.json"
    )
